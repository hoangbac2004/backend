...
Client::Client(const Client& other)
    : number_(other.number_), username(other.username), password(other.password) {}

Client::Client(Client&& old) noexcept
    : number_(std::move(old.number_)),
...
void* Client::operator new(size_t size) { return oracle::occi::PObject::operator new(size); }
...
void* Client::operator new(size_t size, const oracle::occi::Connection* sess,
  ...
  return oracle::occi::PObject::operator new(size, sess, table,
                                             std::format("{}.{}", schema, type_name).c_str());
}
...
void Client::getSQLTypeName(oracle::occi::Environment* env, void** schemaName,
  ...
  oracle::occi::PObject::getSQLTypeName(env, readSQL, schemaName, schemaNameLen, typeName,
                                        typeNameLen);
}
...
void Client::readSQL(oracle::occi::AnyData& streamOCCI_) noexcept {
  number_ = static_cast<long long>((double)streamOCCI_.getNumber());
...
void Client::writeSQL(oracle::occi::AnyData& streamOCCI_) noexcept {
  streamOCCI_.setNumber(static_cast<double>(number_));
